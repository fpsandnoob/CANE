...
class Model:
    ...
    def newconv(self):
        ...
        conv1_A_activate = conv1_1_A * tf.sigmoid(conv1_2_A)
        conv1_B_activate = conv1_1_B * tf.sigmoid(conv1_2_B)
        conv1_NEG_activate = conv1_1_NEG * tf.sigmoid(conv1_2_NEG)
        ...
        conv2_A_activate = conv2_1_A * tf.sigmoid(conv2_2_A)
        conv2_B_activate = conv2_1_B * tf.sigmoid(conv2_2_B)
        conv2_NEG_activate = conv2_1_NEG * tf.sigmoid(conv2_2_NEG)
    ...
    def conv(self):
        W2 = tf.Variable(tf.truncated_normal([2, int(config.embed_size / 2), 1, 100], stddev=0.3))
        rand_matrix = tf.Variable(tf.truncated_normal([100, 100], stddev=0.3))
        ...
        # att1 = tf.nn.relu(att1)
        # att3 = tf.nn.relu(att3)
