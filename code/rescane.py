...
class Modelv3:
    ...
    def compute_all_loss(self):
        ...
        p1 = tf.log(tf.sigmoid(tf.reduce_sum(tf.multiply(self.merge_A, self.merge_B), 1)) + 0.001)

        p2 = tf.log(tf.sigmoid(-tf.reduce_mean(tf.multiply(self.merge_A, self.merge_NEG), 1)) + 0.001)
        ...
        p5 = tf.log(tf.sigmoid(tf.reduce_mean(tf.multiply(self.merge_B, self.N_A), 1)) + 0.001)

        p6 = tf.log(tf.sigmoid(tf.reduce_mean(tf.multiply(self.merge_NEG, self.N_A), 1)) + 0.001)

        p7 = tf.log(tf.sigmoid(tf.reduce_mean(tf.multiply(self.N_B, self.merge_A), 1)) + 0.001)

        p8 = tf.log(tf.sigmoid(tf.reduce_mean(tf.multiply(self.N_B, self.merge_NEG), 1)) + 0.001)
...
class Modelv5:
    ...
    def autoencoder(self):
        with tf.name_scope('autoencoder') as scope:
            ...
            def residual_blocks(x: tf.Tensor, weight: tf.Variable, biase: tf.Variable) -> tf.Tensor:
                x_ = tf.nn.relu(tf.layers.batch_normalization(x))
                x_ = tf.nn.xw_plus_b(x_, weight, biase)
                return tf.add(x_, x)

            def encoder(x: tf.Tensor) -> tf.Tensor:
                with tf.name_scope("encoder") as scope:
                    layer_1 = residual_blocks(x, weights['encoder_w1'], biases['encoder_b1'])
                    layer_2 = residual_blocks(layer_1, weights['encoder_w2'], biases['encoder_b2'])
                    layer_3 = residual_blocks(layer_2, weights['encoder_w3'], biases['encoder_b3'])
                    return layer_3

            def decoder(x: tf.Tensor) -> tf.Tensor:
                with tf.name_scope("decoder") as scope:
                    layer_1 = residual_blocks(x, weights['decoder_w1'], biases['decoder_b1'])
                    layer_2 = residual_blocks(layer_1, weights['decoder_w2'], biases['decoder_b2'])
                    layer_3 = residual_blocks(layer_2, weights['decoder_w3'], biases['decoder_b3'])
                    return layer_3
