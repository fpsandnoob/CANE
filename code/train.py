...
def autoencoder_train():
    with tf.Graph().as_default():
        sess = tf.Session()
        with sess.as_default():
            model = rescane.Modelv3(data.num_vocab, data.num_nodes)
            opt = tf.train.AdagradOptimizer(config.lr)
            train_op = opt.minimize(model.loss)
            train_op_autoencoder = opt.minimize(model.compute_autoencoder_loss())
            sess.run(tf.global_variables_initializer())
            saver = tf.train.Saver()
            # training
            print('start training.......')
            saver.recover_last_checkpoints("./ckpt/model.ckpt")
            # for epoch in range(int(config.num_epoch / 2)):
            #     if epoch % 10 == 0:
            #         saver.save(sess, "./ckpt/model.ckpt")
            #     loss_epoch = 0
            #     batches = data.generate_batches()
            ...
            #             model.sentiment_neg: sen3,
            #         }
            #         _, loss_batch = sess.run([train_op_autoencoder, model.autoencoder_loss], feed_dict=feed_dict)
            #         sys.stdout.flush()
            #         sys.stdout.write("autoenocoder: batch: {}/{}, loss: {}\r".format(i, num_batch, loss_batch))
            #         loss_epoch += loss_batch
            #     t2 = time.time()
            #     print('autoencoder: epoch: ', epoch + 1, ' loss: ', loss_epoch, 'cost time: ', t2 - t1)
            for epoch in range(int(config.num_epoch / 2)):
                if epoch % 10 == 0:
                    saver.save(sess, "./ckpt/model.ckpt")
                loss_epoch = 0
                ...
                for i in range(num_batch):
                    ...
                    _, loss_batch = sess.run([train_op, model.loss], feed_dict=feed_dict)
                    sys.stdout.flush()
                    sys.stdout.write("all: batch: {}/{}, loss: {}\r".format(i, num_batch, loss_batch))
                    loss_epoch += loss_batch
                t2 = time.time()
                print('all: epoch: ', epoch + 1, ' loss: ', loss_epoch, 'cost time: ', t2 - t1)
            file = open('embed.txt', 'w')
            batches = data.generate_batches(mode='add')
            num_batch = len(batches)
            embed = [[] for _ in range(data.num_nodes)]
